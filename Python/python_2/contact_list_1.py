...
def contact_add(n, c, t, e, g=None):
    ...
    new_contact = {"nome": n, "cognome": c, "telefono": t, "email": e, "gruppo": g}
...
def create_group(n, d):
    global groups
    # Aggiungere soltanto se il nome non esiste gia' il nome del dizionario
...
def ui_ask_what_to_do():
    while True:
        print(
            "\nScegli un numero o scrivi l'azione:\n"
            + "\r\t1. add: Aggiungi un nuovo contatto\n"
            + "\r\t2. show: Mostra tutta la contacts\n"
            + "\r\t3. search: Cerca un contatto\n"
            + "\r\t4. group_C: Crea un gruppo\n"
            + "\r\t5. group_S: Cerca contatto in un gruppo\n"
            + "\r\t6. exit: Esci\n"
        )

        choice = (
            input("Scrivi l'azione oppure digita il numero corrispondente:\n")
            .strip()
            .lower()
        )
        ...
        if choice == "add" or choice == "1":
            ...
            ui_search_contact()
        elif choice == "group_C" or choice == "4":
            create_group(
