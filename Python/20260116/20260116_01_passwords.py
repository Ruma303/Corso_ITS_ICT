...
def password_generator(alfa, /, # Tipo di alfabeto (es solo numeri, solo speciali etc.)
                      ...
                      min_len=8, max_len=8, # Lunghezza min - max dell'intera password
                      ...
                      min_nums=1, max_nums=1, # Quantità min - max di numeri
                      min_char_lower=1, max_char_lower=1, # Quantità min - max di caratteri minuscoli
                      min_char_upper=1, max_char_upper=1, # Quantità min - max di caratteri maiuscoli
                      min_special=1, max_special=1 # Quantità min - max di caratteri speciali
                      ):
  ...
  try:
    if password_length < 8 or min_len < 8:
      raise Exception(f"Per ragioni di sicurezza la lunghezza della password dev'essere di 8 o più caratteri")

    if current_password_length > max_len:
        raise Exception(f"L'insieme dei vincoli inseriti {current_password_length} supera la lunghezza della password da generare {max_len}")

    if min_len < current_password_length < max_len:
      raise Exception(f"L'insieme dei vincoli inseriti {current_password_length} dev'essere compreso tra {min_len} e {max_len}")
  ...
  # Creazione password effettiva
  password = ""

  # Fare lo shuffle della lista alla fine

  for _ in range(0, password_length):
    char_type = rand_char(alfa)

    if can_nums is False:
      continue
